...
def response_path(path, base_dir = "webroot"):
    """
    ...
    # under webroot.
    requested_content = base_dir + path

    if not os.path.exists(requested_content):
        raise NameError

    if os.path.isdir(requested_content):
        mime_type = b"text/plain"
        content = "\r\n".join(os.listdir(requested_content)).encode()

    else:
        mime_type = mimetypes.guess_type(requested_content)[0].encode()
        with open(requested_content, "rb") as file_contents:
